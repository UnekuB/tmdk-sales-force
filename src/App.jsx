...
const DEFAULT_APPS_SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbwQIVyacsmcIhySMROBUXTtOuGpH-GO1scyU5L7za__mFRLKYL0ttK8hJk5EeWZROuo5g/exec";
const DEFAULT_LOGO_URL =
  "https://drive.google.com/thumbnail?id=15ZeFX5OUlXVKvJt409-WpiYzcevUUG-l&sz=w1000";
...
function defaultManagerForm() {
  return { name: "", role: "ASM", region: "", statesText: "", email: "", phone: "", reportsTo: "", active: true };
}
...
const seedCache = normalizeDb({
  managers: [
    ...
    withMeta(
      {
        ...
        role: "RSM",
        region: "North West",
        states: ["Kaduna", "Katsina", "Kano", "Jigawa", "Sokoto", "Kebbi", "Zamfara"],
        email: "rsm.nw@tmdk.com",
        phone: "08030000003",
        reportsTo: "M004",
        active: true,
        createdAt: nowISO(),
    ...
    withMeta(
      {
        id: "M005",
        name: "Sales Operations Admin",
        role: "Sales Operations Admin",
        region: "All Regions",
        states: [],
        email: "salesops@tmdk.com",
        phone: "08030000005",
        reportsTo: "M004",
        active: true,
        createdAt: nowISO(),
    ...
    withMeta(
      {
        ...
        contactPerson: "Musa Aliyu",
        phone: "08040000001",
        email: "musa@gidangona.com",
        segment: "Prospect",
        ownerManagerId: "M001",
        active: true,
        createdAt: nowISO(),
    ...
    withMeta(
      {
        ...
        plannedAtStartOfDay: true,
        visitType: "Routine Visit",
        objective: "Introduce product and assess feed demand",
        outcome: "Prospecting",
        comments: "Customer requested price list.",
        orderMade: false,
        orderQtyMt: 0,
        orderValueNgn: 0,
        pipelineStatus: "Follow-up",
        nextActionDate: addDays(todayISO(), 5),
...
function runSelfTests() {
  ...
  const merged = mergeCollections(
    [{ id: "X1", updatedAt: "2026-01-01T00:00:00.000Z", value: 1 }],
    [{ id: "X1", updatedAt: "2026-01-02T00:00:00.000Z", value: 2 }]
  );
...
async function backendLogin(settings, email, password) {
  const result = await backendPost(settings, { action: "login", email, password });
  return result.data;
}
async function backendLogout(settings, session) {
  return backendPost(settings, { action: "logout", token: session?.token || "" });
}
async function backendRead(settings, session) {
  const result = await backendGet(settings, "read", session);
  return normalizeDb(result.data);
}
async function backendMerge(settings, session, db) {
  return backendPost(settings, {
    action: "merge",
    token: session?.token || "",
    data: {
      managers: sanitizeRows(db.managers),
      customers: sanitizeRows(db.customers),
      visits: sanitizeRows(db.visits),
      auditLog: sanitizeRows(db.auditLog),
      authUsers: sanitizeRows(db.authUsers),
    },
  });
}
async function backendChangePassword(settings, session, currentPassword, newPassword) {
  return backendPost(settings, {
...
function Button({ children, onClick, secondary = false, disabled = false, type = "button" }) {
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      style={{ ...(secondary ? styles.buttonSecondary : styles.button), opacity: disabled ? 0.6 : 1 }}
    >
      {children}
    </button>
  );
...
function LogoBlock({ logoUrl, compact = false }) {
  const fallbackUrl = DEFAULT_LOGO_URL;
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: compact ? 10 : 14 }}>
      ...
      <div>
        <div style={{ fontWeight: 800, fontSize: compact ? 18 : 22, letterSpacing: "-0.02em" }}>TMDK Sales Force</div>
        <div style={{ color: "#64748b", fontSize: compact ? 12 : 13 }}>Pipeline, field execution, and manager intelligence</div>
      </div>
    </div>
  );
}
    ...
    <div style={styles.page}>
      <div style={{ ...styles.container, maxWidth: 1080, minHeight: "80vh", justifyContent: "center" }}>
        <div style={{ ...styles.grid2, alignItems: "stretch" }}>
          ...
          <Card>
            <div style={{ fontSize: 12, color: "#16a34a", fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase" }}>
              Secure sign in
            </div>
            ...
            <div style={{ display: "grid", gap: 12, marginTop: 14 }}>
              <Field label="Email">
                <input style={styles.input} value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Enter your email" />
              </Field>
              ...
              {status ? (
                <div style={{ border: "1px solid #ecfdf5", borderRadius: 12, padding: 12, background: "#f8fafc" }}>{status}</div>
              ) : null}
              <Button
                onClick={async () => {
                  try {
    ...
    <Card>
      ...
      <div style={styles.grid2}>
        <Field label="Date From">
          <input style={styles.input} type="date" value={filters.from} onChange={(e) => setFilters((f) => ({ ...f, from: e.target.value }))} />
        </Field>
        <Field label="Date To">
          <input style={styles.input} type="date" value={filters.to} onChange={(e) => setFilters((f) => ({ ...f, to: e.target.value }))} />
        </Field>
        <Field label="Region">
        ...
        <Field label="State">
          <select style={styles.select} value={filters.state} onChange={(e) => setFilters((f) => ({ ...f, state: e.target.value }))}>
            <option value="all">All states</option>
            {availableStates.map((state) => <option key={state} value={state}>{state}</option>)}
          </select>
        </Field>
        <Field label="Manager">
          <select style={styles.select} value={filters.managerId} onChange={(e) => setFilters((f) => ({ ...f, managerId: e.target.value }))}>
...
function Dashboard({ visits, customers, managers, currentManager, filters }) {
  ...
  const enrichedVisits = activeVisits.map((v) => ({
    ...v,
    manager: managers.find((m) => m.id === v.managerId),
  ...
  const byPipeline = PIPELINE_STATUSES.map((status) => ({
    label: status,
    value: filteredVisits.filter((v) => v.pipelineStatus === status).length,
  }));

  const byManager = visibleManagerIds
    .map((id) => {
  ...
  const territoryRows = Array.from(new Set(activeCustomers.map((c) => `${c.region}|||${c.state}`)))
    .map((key) => {
      ...
      return {
        ...
        visits: territoryVisits.length,
        converted: territoryConverted,
        conversionPct: territoryVisits.length ? ((territoryConverted / territoryVisits.length) * 100).toFixed(1) : "0.0",
        mt: territoryMt,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={styles.grid2}>
        <Card>
          <h3 style={{ marginTop: 0 }}>Role Access Summary</h3>
          <div style={{ color: "#475569", display: "grid", gap: 8 }}>
            <div><strong>ASM:</strong> sees owned or state-covered customers and related visits.</div>
            <div><strong>RSM:</strong> sees all customers and visits within assigned region.</div>
            <div><strong>NSM / Sales Ops Admin:</strong> sees everything.</div>
          </div>
        </Card>
        <Card>
      ...
      <Card>
        ...
        <div style={{ display: "grid", gap: 10 }}>
          {insightBullets.map((insight, index) => (
            <div key={index} style={{ display: "flex", gap: 10, alignItems: "flex-start", color: "#475569" }}>
              <div style={{ width: 10, height: 10, borderRadius: 999, background: "#22c55e", marginTop: 6 }} />
              <div>{insight}</div>
            </div>
          ))}
        </div>
    ...
    setForm(
      editing
        ? {
            ...
            statesText: (editing.states || []).join(", "),
            email: editing.email,
            phone: editing.phone,
  ...
  const saveManager = () => {
    setDb((db) => {
      ...
      const payload = withMeta(
        {
          id: editing?.id || generateId("M", db.managers),
          name: form.name,
          role: form.role,
          region: form.region,
          states: splitCsv(form.statesText),
          email: form.email,
          phone: form.phone,
      ...
      let nextDb = {
        ...db,
        managers: editing ? db.managers.map((m) => (m.id === editing.id ? payload : m)) : [...db.managers, payload],
      };
      if (!editing && !existingAuth && form.email) {
        const authUser = withMeta(
          {
            id: generateId("U", db.authUsers),
            email: form.email,
            password: "1234",
  ...
  const removeManager = (id) => {
    ...
    setDb((db) =>
      addAuditEntry(
        {
          ...db,
          managers: db.managers.map((m) =>
            m.id === id ? { ...m, active: false, deletedAt: nowISO(), deletedBy: session.email, updatedAt: nowISO(), updatedBy: session.email } : m
          ),
          authUsers: db.authUsers.map((u) =>
            u.managerId === id ? { ...u, active: false, deletedAt: nowISO(), deletedBy: session.email, updatedAt: nowISO(), updatedBy: session.email } : u
          ),
          visits: db.visits.map((v) =>
            v.managerId === id ? { ...v, deletedAt: nowISO(), deletedBy: session.email, updatedAt: nowISO(), updatedBy: session.email } : v
          ),
        },
        session,
        "delete",
        "manager",
        id,
        `Soft-deleted manager ${target?.name || id}`
      )
    );
  };
    ...
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
          <div>
            <h3 style={{ margin: 0 }}>Managers Master</h3>
            <div style={{ color: "#64748b", fontSize: 14 }}>Manage ASM, RSM, NSM and Sales Operations Admin profiles.</div>
          </div>
          {canManage ? <Button onClick={() => { setEditing(null); setOpen(true); }}>Add Manager</Button> : null}
        </div>
        <div style={styles.tableWrap}>
          <table style={styles.table}>
            <thead>
              <tr>
                {["ID", "Name", "Role", "Region", "States", "Reports To", "Status"].map((h) => <th key={h} style={styles.th}>{h}</th>)}
                {canManage ? <th style={styles.th}>Actions</th> : null}
              </tr>
            </thead>
            <tbody>
              {activeManagers.map((m) => (
                <tr key={m.id}>
                  ...
                  <td style={styles.td}><Badge>{m.role}</Badge></td>
                  <td style={styles.td}>{m.region}</td>
                  <td style={styles.td}>{(m.states || []).join(", ")}</td>
                  <td style={styles.td}>{activeManagers.find((x) => x.id === m.reportsTo)?.name || "—"}</td>
                  <td style={styles.td}>{m.active ? "Active" : "Inactive"}</td>
                  {canManage ? (
                    <td style={styles.td}>
                      <div style={{ display: "flex", gap: 8 }}>
                        <Button secondary onClick={() => { setEditing(m); setOpen(true); }}>Edit</Button>
                        <Button secondary onClick={() => removeManager(m.id)}>Delete</Button>
                      </div>
                    </td>
                  ) : null}
                </tr>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? "Edit Manager" : "Add Manager"}>
        <div style={styles.grid2}>
          ...
          <Field label="Reports To">
            <select style={styles.select} value={form.reportsTo || ""} onChange={(e) => setForm({ ...form, reportsTo: e.target.value })}>
              <option value="">None</option>
              {activeManagers.filter((m) => m.id !== editing?.id).map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}
            </select>
          </Field>
          <Field label="Active">
...
function CustomersTab({ customers, managers, currentManager, setDb, session }) {
    ...
    setForm(
      editing
        ? {
            ...
            ownerManagerId: editing.ownerManagerId || "",
            active: editing.active,
          }
        : defaultCustomerForm()
    );
  ...
  const saveCustomer = () => {
    setDb((db) => {
      ...
      const payload = withMeta(
        { id: editing?.id || generateId("C", db.customers), ...form, ownerManagerId: ownerId, createdAt: editing?.createdAt || nowISO() },
        session.email
      );
      return addAuditEntry(
        { ...db, customers: editing ? db.customers.map((c) => (c.id === editing.id ? payload : c)) : [...db.customers, payload] },
        session,
        editing ? "update" : "create",
        "customer",
        payload.id,
        `${editing ? "Updated" : "Created"} customer ${payload.name}`
      );
    });
    setOpen(false);
    setEditing(null);
      ...
      addAuditEntry(
        {
          ...
          customers: db.customers.map((c) =>
            c.id === id ? { ...c, active: false, deletedAt: nowISO(), deletedBy: session.email, updatedAt: nowISO(), updatedBy: session.email } : c
          ),
          visits: db.visits.map((v) =>
            v.customerId === id ? { ...v, deletedAt: nowISO(), deletedBy: session.email, updatedAt: nowISO(), updatedBy: session.email } : v
          ),
        },
        session,
        "delete",
        "customer",
        id,
        `Soft-deleted customer ${target?.name || id}`
      )
    );
  };
    ...
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
          <div>
            <h3 style={{ margin: 0 }}>Customer Master</h3>
            <div style={{ color: "#64748b", fontSize: 14 }}>Track direct and indirect sales customers.</div>
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <input style={styles.input} placeholder="Search customers" value={search} onChange={(e) => setSearch(e.target.value)} />
            {canManage ? <Button onClick={() => { setEditing(null); setOpen(true); }}>Add Customer</Button> : null}
          </div>
        </div>
        ...
        <div style={styles.tableWrap}>
          <table style={styles.table}>
            <thead>
              <tr>
                {["ID", "Name", "Type", "Region", "State", "Sales Type", "Segment", "Owner", "Status"].map((h) => (
                  <th key={h} style={styles.th}>{h}</th>
                ))}
                {canManage ? <th style={styles.th}>Actions</th> : null}
              </tr>
            ...
            <tbody>
              {rows.map((c) => (
                <tr key={c.id}>
                  ...
                  <td style={styles.td}>{c.salesType}</td>
                  <td style={styles.td}>{c.segment}</td>
                  <td style={styles.td}>{managers.find((m) => m.id === c.ownerManagerId)?.name || "—"}</td>
                  <td style={styles.td}>{c.active ? "Active" : "Inactive"}</td>
                  {canManage ? (
                    <td style={styles.td}>
                      <div style={{ display: "flex", gap: 8 }}>
                        <Button secondary onClick={() => { setEditing(c); setOpen(true); }}>Edit</Button>
                        <Button secondary onClick={() => removeCustomer(c.id)}>Delete</Button>
                      </div>
                    </td>
                  ) : null}
                </tr>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? "Edit Customer" : "Add Customer"}>
        <div style={styles.grid2}>
          ...
          <Field label="Sales Type">
            <select style={styles.select} value={form.salesType} onChange={(e) => setForm({ ...form, salesType: e.target.value })}>
              {SALES_TYPES.map((type) => <option key={type} value={type}>{type}</option>)}
            </select>
          </Field>
          ...
          <Field label="Owner Manager">
            <select style={styles.select} value={form.ownerManagerId} onChange={(e) => setForm({ ...form, ownerManagerId: e.target.value })}>
              <option value="">Unassigned</option>
              {getVisibleManagers(managers, currentManager).map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}
            </select>
          </Field>
          <Field label="Region"><input style={styles.input} value={form.region} onChange={(e) => setForm({ ...form, region: e.target.value })} /></Field>
...
function VisitsTab({ visits, customers, managers, setDb, currentManager, session }) {
  ...
  const rows = visits
    ...
    .map((v) => ({ ...v, manager: managers.find((m) => m.id === v.managerId), customer: customers.find((c) => c.id === v.customerId) }))
    ...
    setForm(
      editing
        ? {
            ...
            plannedAtStartOfDay: editing.plannedAtStartOfDay,
            visitType: editing.visitType,
            objective: editing.objective,
            outcome: editing.outcome,
            comments: editing.comments,
            orderMade: editing.orderMade,
            orderQtyMt: editing.orderQtyMt || 0,
            orderValueNgn: editing.orderValueNgn,
            pipelineStatus: editing.pipelineStatus,
            nextActionDate: editing.nextActionDate,
            visitStatus: editing.visitStatus,
            salesType: editing.salesType,
          }
        : defaultVisitForm()
    );
  ...
  const saveVisit = () => {
    setDb((db) => {
      ...
      return addAuditEntry(
        { ...db, visits: editing ? db.visits.map((v) => (v.id === editing.id ? payload : v)) : [...db.visits, payload] },
        session,
        editing ? "update" : "create",
        "visit",
        payload.id,
        `${editing ? "Updated" : "Created"} visit ${payload.id}`
      );
    });
    setOpen(false);
    setEditing(null);
      ...
      addAuditEntry(
        {
          ...db,
          visits: db.visits.map((v) =>
            v.id === id ? { ...v, deletedAt: nowISO(), deletedBy: session.email, updatedAt: nowISO(), updatedBy: session.email } : v
          ),
        },
        session,
        "delete",
  ...
  const bulkSave = () => {
    ...
    setDb((db) => {
      ...
      const newVisits = bulkSelected.map((customerId) => {
        ...
        const visit = withMeta(
          {
            id: nextId,
            visitDate: bulkVisitDate,
            managerId: bulkManagerId,
            customerId,
            plannedAtStartOfDay: true,
            visitType: bulkVisitType,
            ...
            orderValueNgn: 0,
            pipelineStatus: "Prospecting",
            nextActionDate: bulkVisitDate,
            visitStatus: "Planned",
            salesType: customer?.salesType || "Direct",
            createdAt: nowISO(),
          },
          session.email
        );
      ...
      return addAuditEntry(
        { ...db, visits: [...db.visits, ...newVisits] },
        session,
        "bulk_create",
        "visit",
        newVisits.map((v) => v.id).join(", "),
        `Bulk planned ${newVisits.length} visits`
      );
    });
    setBulkOpen(false);
    setBulkSelected([]);
    ...
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
          <div>
            ...
            <div style={{ color: "#64748b", fontSize: 14 }}>Plan visits and track conversion pipeline.</div>
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <input style={styles.input} placeholder="Search visits" value={search} onChange={(e) => setSearch(e.target.value)} />
            {canManage ? <Button secondary onClick={() => setBulkOpen(true)}>Bulk Plan</Button> : null}
            {canManage ? <Button onClick={() => { setEditing(null); setOpen(true); }}>Add Visit</Button> : null}
          </div>
        </div>
        ...
        <div style={styles.tableWrap}>
          <table style={styles.table}>
            <thead>
              <tr>
                {["Date", "Manager", "Customer", "Visit Type", "Pipeline", "Order", "Order MT", "Order Value", "Status"].map((h) => (
                  <th key={h} style={styles.th}>{h}</th>
                ))}
                {canManage ? <th style={styles.th}>Actions</th> : null}
              </tr>
            ...
            <tbody>
              {rows.map((v) => (
                <tr key={v.id}>
                  ...
                  <td style={styles.td}>{v.customer?.name}</td>
                  <td style={styles.td}>{v.visitType}</td>
                  <td style={styles.td}><Badge>{v.pipelineStatus}</Badge></td>
                  <td style={styles.td}>{v.orderMade ? "Yes" : "No"}</td>
                  <td style={styles.td}>{Number(v.orderQtyMt || 0).toFixed(2)} MT</td>
                  <td style={styles.td}>{currency(v.orderValueNgn)}</td>
                  <td style={styles.td}>{v.visitStatus}</td>
                  {canManage ? (
                    <td style={styles.td}>
                      <div style={{ display: "flex", gap: 8 }}>
                        <Button secondary onClick={() => { setEditing(v); setOpen(true); }}>Edit</Button>
                        <Button secondary onClick={() => removeVisit(v.id)}>Delete</Button>
                      </div>
                    </td>
                  ) : null}
                </tr>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? "Edit Visit" : "Add Visit"}>
        <div style={styles.grid2}>
          ...
          <Field label="Manager">
            <select style={styles.select} value={form.managerId} onChange={(e) => setForm({ ...form, managerId: e.target.value })}>
              <option value="">Select manager</option>
              {visibleManagers.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}
            </select>
          </Field>
          <Field label="Customer">
            <select
              ...
              onChange={(e) => {
                ...
                const customer = activeCustomers.find((c) => c.id === v);
                setForm({ ...form, customerId: v, salesType: customer?.salesType || form.salesType });
              }}
            >
          ...
          <Field label="Visit Type">
            <select style={styles.select} value={form.visitType} onChange={(e) => setForm({ ...form, visitType: e.target.value })}>
          ...
          <Field label="Pipeline Status">
            <select style={styles.select} value={form.pipelineStatus} onChange={(e) => setForm({ ...form, pipelineStatus: e.target.value })}>
              {PIPELINE_STATUSES.map((status) => <option key={status} value={status}>{status}</option>)}
            </select>
          </Field>
      ...
      <Modal open={bulkOpen} onClose={() => setBulkOpen(false)} title="Bulk Visit Planning">
        <div style={styles.grid2}>
          <Field label="Manager">
            <select style={styles.select} value={bulkManagerId} onChange={(e) => setBulkManagerId(e.target.value)}>
              <option value="">Select manager</option>
              {visibleManagers.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}
            </select>
          </Field>
          ...
          <Field label="Visit Type">
            <select style={styles.select} value={bulkVisitType} onChange={(e) => setBulkVisitType(e.target.value)}>
              {VISIT_TYPES.map((type) => <option key={type} value={type}>{type}</option>)}
            </select>
          </Field>
        ...
        <div style={{ marginTop: 16, ...styles.tableWrap }}>
          <table style={styles.table}>
            <thead>
              <tr>
                {["Select", "Customer", "State", "Region", "Sales Type"].map((h) => <th key={h} style={styles.th}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {activeCustomers.map((c) => (
                <tr key={c.id}>
                  <td style={styles.td}><input type="checkbox" checked={bulkSelected.includes(c.id)} onChange={() => toggleBulk(c.id)} /></td>
                  <td style={styles.td}>{c.name}</td>
                  <td style={styles.td}>{c.state}</td>
                  <td style={styles.td}>{c.region}</td>
                  <td style={styles.td}>{c.salesType}</td>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 16 }}>
          <div style={{ color: "#64748b" }}>{bulkSelected.length} customer(s) selected</div>
          <div style={{ display: "flex", gap: 8 }}>
            <Button secondary onClick={() => setBulkOpen(false)}>Cancel</Button>
            <Button onClick={bulkSave}>Create Planned Visits</Button>
          </div>
        </div>
      </Modal>
    </div>
      ...
      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
          <div>
            ...
            <div style={{ color: "#64748b", fontSize: 14 }}>Every important action is logged.</div>
          </div>
          <input style={styles.input} placeholder="Search audit trail" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        <div style={styles.tableWrap}>
          <table style={styles.table}>
            <thead>
              <tr>
                {["Time", "Actor", "Role", "Action", "Entity", "Summary"].map((h) => <th key={h} style={styles.th}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {rows.map((a) => (
                <tr key={a.id}>
                  ...
                  <td style={styles.td}>{a.actorName}</td>
                  <td style={styles.td}>{a.actorRole}</td>
                  <td style={styles.td}><Badge>{a.action}</Badge></td>
                  <td style={styles.td}>{a.entityType} • {a.entityId}</td>
                  <td style={styles.td}>{a.summary}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
function UsersTab({ authUsers, managers, setDb, session }) {
  ...
  useEffect(() => {
    setForm(
      editing
        ? { email: editing.email, password: editing.password || "1234", managerId: editing.managerId, role: editing.role, name: editing.name, active: editing.active !== false }
        : { email: "", password: "1234", managerId: "", role: "ASM", name: "", active: true }
    );
  }, [editing, open]);
  ...
  const saveUser = () => {
    ...
    setDb((db) => {
      ...
      const payload = withMeta(
        {
          id: editing?.id || generateId("U", db.authUsers),
          email: form.email,
          password: form.password,
          managerId: form.managerId,
          role: form.role,
          name: form.name || manager?.name || form.email,
          active: form.active,
          createdAt: editing?.createdAt || nowISO(),
        },
        session.email
      ...
      return addAuditEntry(
        { ...db, authUsers: editing ? db.authUsers.map((u) => (u.id === editing.id ? payload : u)) : [...db.authUsers, payload] },
        session,
        editing ? "update" : "create",
        "auth_user",
        payload.id,
        `${editing ? "Updated" : "Created"} auth user ${payload.email}`
      );
    });
    setOpen(false);
    setEditing(null);
  ...
  const disableUser = (id) => {
    ...
    setDb((db) =>
      addAuditEntry(
        {
          ...db,
          authUsers: db.authUsers.map((u) => (u.id === id ? { ...u, active: false, updatedAt: nowISO(), updatedBy: session.email } : u)),
        },
        session,
        "disable",
        "auth_user",
        id,
        `Disabled auth user ${target?.email || id}`
      )
    );
  };
  ...
  const resetPasswordLocal = (id) => {
    ...
    setDb((db) =>
      addAuditEntry(
        {
          ...db,
          authUsers: db.authUsers.map((u) => (u.id === id ? { ...u, password: nextPassword, updatedAt: nowISO(), updatedBy: session.email } : u)),
        },
        session,
        "reset_password_local",
        "auth_user",
        id,
        `Locally reset password for ${target?.email || id}`
      )
    );
  };
      ...
      <Card>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
          <div>
            ...
            <div style={{ color: "#64748b", fontSize: 14 }}>Manage login users and map them to managers.</div>
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <input style={styles.input} placeholder="Search users" value={search} onChange={(e) => setSearch(e.target.value)} />
        ...
        <div style={styles.tableWrap}>
          <table style={styles.table}>
            <thead>
              <tr>
                {["Name", "Email", "Role", "Manager", "Status"].map((h) => <th key={h} style={styles.th}>{h}</th>)}
            ...
            <tbody>
              {activeUsers.map((u) => (
                <tr key={u.id}>
                  ...
                  <td style={styles.td}><Badge>{u.role}</Badge></td>
                  <td style={styles.td}>{activeManagers.find((m) => m.id === u.managerId)?.name || "—"}</td>
                  <td style={styles.td}>{u.active ? "Active" : "Inactive"}</td>
                  <td style={styles.td}>
                    <div style={{ display: "flex", gap: 8 }}>
                      <Button secondary onClick={() => { setEditing(u); setOpen(true); }}>Edit</Button>
                      <Button secondary onClick={() => resetPasswordLocal(u.id)}>Reset Password</Button>
                      <Button secondary onClick={() => disableUser(u.id)}>Disable</Button>
                    </div>
                  </td>
                </tr>
              ))}
...
function ExportTab({ managers, customers, visits, auditLog, authUsers, settings, onPull, onPush, syncStatus, syncing, session }) {
  const visitRows = visits
    ...
    .map((v) => {
      ...
      return {
        ...
        Region: customer?.region || "",
        State: customer?.state || "",
        Owner_Manager_ID: customer?.ownerManagerId || "",
        Sales_Type: v.salesType,
        Visit_Type: v.visitType,
        Planned_At_Start_Of_Day: v.plannedAtStartOfDay,
        Objective: v.objective,
        Outcome: v.outcome,
        Comments: v.comments,
        Order_Made: v.orderMade,
        Order_Qty_MT: v.orderQtyMt,
        Order_Value_NGN: v.orderValueNgn,
        Pipeline_Status: v.pipelineStatus,
        Next_Action_Date: v.nextActionDate,
    ...
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <Card>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 16 }}>
          <Button secondary onClick={onPull} disabled={syncing}>Pull & Merge</Button>
          <Button onClick={onPush} disabled={syncing}>Push Incremental Merge</Button>
        </div>
...
export default function TMDKSalesForceApp() {
  ...
  if (!session || !currentManager) {
    return <LoginScreen settings={settings} onLogin={setSession} />;
  }
    ...
    <div style={styles.page}>
      <div style={styles.container}>
        <AppHeader
          ...
          onLogout={async () => {
            try {
              await backendLogout(settings, session);
            } catch (_) {}
            setSession(null);
          }}
          ...
          {[
            ...
            <Card key={kpi.label}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ color: "#64748b", fontSize: 13 }}>{kpi.label}</div>
                <div style={{ width: 34, height: 34, borderRadius: 12, background: "#f0fdf4", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16 }}>
                  {kpi.icon}
                </div>
              </div>
              <div style={{ fontSize: 24, fontWeight: 700, marginTop: 10 }}>{kpi.value}</div>
            </Card>
          ))}
        </div>
        ...
        <Filters filters={filters} setFilters={setFilters} managers={visibleManagers} customers={getVisibleCustomers(db.customers, db.managers, currentManager)} />
        ...
        {tab === "exports" && isAdmin ? <ExportTab managers={db.managers} customers={db.customers} visits={db.visits} auditLog={db.auditLog} authUsers={db.authUsers} settings={settings} onPull={refreshFromBackend} onPush={pushToBackend} syncStatus={syncStatus} syncing={syncing} session={session} /> : null}
